...
//! # Isolation
//!
//! Each test owns a private server on a display number `Xvfb` itself picks and
//! reports, so concurrent test binaries and the developer's own session never
//! collide. The server is torn down by [`XvfbServer`]'s `Drop`, so it dies on
//! panic too.
//!
//! # Time
//!
//! The server is a real OS process, so waiting for it to accept connections
//! cannot be virtual. Every wait is bounded against an explicit deadline --
//! the server's own readiness report, or a polled observable -- and never a
//! fixed sleep used as synchronisation: a regression that would hang the run
...
/// on its observable, not on elapsed time.
